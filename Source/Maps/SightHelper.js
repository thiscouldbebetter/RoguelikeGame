...
class SightHelper {
    constructor(distanceFromEyeMax) {
        ...
        var numberOfCellPositionsMax = 4
            * distanceFromEyeMax
            * distanceFromEyeMax;
    ...
    copyNCellsAtPositionsFromMapToOther(numberOfCellsToCopy, cellPositionsToCopy, mapFrom, mapTo) {
        for (var i = 0; i < numberOfCellsToCopy; i++) {
            ...
            if (cellToCopy != null) {
                ...
                this.overwriteMapCellWithPerceptsFromOther(cellToOverwrite, cellToCopy);
            }
        }
    }
    overwriteMapCellWithPerceptsFromOther(cellTo, cellFrom) {
        ...
        for (var i = 0; i < entitiesActual.length; i++) {
            ...
            var entityPerceivedProperties = new Array();
            entityPerceivedProperties.push(entityActual.drawable().clone());
            entityPerceivedProperties.push(entityActual.locatable().clone());
            ...
            var entityPerceived = new Entity2(entityActual.name, entityPerceivedProperties);
    ...
    updatePlaceFromCompleteForViewerPosAndRange(placeKnown, placeComplete, viewerPos, sightRange) {
        var cellPositionsVisible = this.cellPositionsVisible(viewerPos, sightRange, placeComplete.map);
        var mapComplete = placeComplete.map;
        var mapKnown = placeKnown.map;
        this.entitiesPerceivedTransientRemoveFromMap(mapKnown);
        this.copyNCellsAtPositionsFromMapToOther(this.numberOfCellsVisible, cellPositionsVisible, mapComplete, mapKnown);
    }
    cellPositionsVisible(eyePos, distanceFromEyeMax, map) {
        var rangeInitial = new RangeExtent(0, 1);
        var returnValues = this.cellPositionsVisibleForRange(eyePos, distanceFromEyeMax, map, rangeInitial);
        return returnValues;
    }
        ...
        for (var r = 1; r <= distanceFromEyeMax; r++) {
            ...
            for (var s = 0; s < this.directionsForSides.length; s++) {
                ...
                for (var d = 0; d < r; d++) {
                    ...
                    var isCellInMapBounds = cellPos.isInRangeMax(map.sizeInCellsMinusOnes);
                    if (isCellInMapBounds) {
                        var cellSpan = new RangeExtent(this.atan3(vertexPosRelative0), this.atan3(vertexPosRelative1));
                        var cellSpanAsSet = new RangeSet([cellSpan]);
                        cellSpanAsSet.splitRangesThatSpanPeriod(1);
                        var doesOverlap = cellSpanAsSet.overlapsWith(angleRangeSetNotYetBlocked);
                        if (doesOverlap) {
                            var cellPosVisible = this._cellPositionsVisible[this.numberOfCellsVisible];
    ...
    lineOfSightBetweenPointsOnMap(point0, point1, map) {
        ...
        var pointsVisibleFromPoint0 = this.cellPositionsVisible(point0, distanceBetweenPoints, map);
