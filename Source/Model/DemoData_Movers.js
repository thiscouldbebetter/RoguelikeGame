...
class DemoData_Movers {
    ...
    buildEntityDefnGroups_MoversAndCorpses(visualsByName, activityDefns, itemCategories) {
        ...
        for (var i = 0; i < agentDatas.length; i++) {
            ...
            var itemDefnCorpse = new ItemDefn(itemDefnName, // name
        ...
        var entityGroupAgents = new EntityDefnGroup("Agents", 0, // relativeFrequency
        entityDefnsMovers);
        var entityGroupCorpses = new EntityDefnGroup("Corpses", 0, // relativeFrequency
        entityDefnsCorpses);
        var agentGroups = [entityGroupAgents];
        var entityDefnGroupsByDifficulty = new Map();
        for (var i = 0; i < entityDefnsMovers.length; i++) {
            ...
            if (difficulty != null) {
                ...
                if (entityDefnGroupForDifficulty == null) {
                    entityDefnGroupForDifficulty = new EntityDefnGroup("AgentsOfDifficulty" + difficulty, 0, // relativeFrequency
                    []);
    ...
    buildAgentDatas() {
        ...
        var agentDatas = [
            ...
            AD.fromName("Gremlin"),
            AD.fromName("Gargoyle"),
            AD.fromName("Winged Gargoyle"),
            // humanoids
            ...
            AD.fromName("Bugbear"),
            AD.fromName("Dwarf Lord"),
            AD.fromName("Dwarf King"),
            AD.fromName("Mind Flayer"),
            AD.fromName("Master Mind Flayer"),
            // minor demons
            //     name, 			df, no, attacks, 						lv,xp, sp,ac,mr, 	al,fq,geno, wt, nt, 	size, resists, chance->resistConveyed
            ...
            new AD("Lemure", 5, one, ["Claw:1d3"], 3, 28, 3, 7, 0, -7, 1, true, 150, 100, medium, null, 0, [sleep, poison]),
            new AD("Quasit", 7, one, ["Claw:2x1d2"], 3, 36, 15, 2, 20, -7, 2, true, 200, 200, small, [poison], .20, [poison]),
            new AD("Tengu", 7, one, ["Bite:1d7"], 6, 76, 13, 5, 30, 7, 3, true, 300, 200, small, [poison], .15, [poison, teleportControl, teleportitis]),
            // jellies
            AD.fromName("Blue Jelly"),
            AD.fromName("Spotted Jelly"),
            AD.fromName("Ochre Jelly"),
            // kobolds
...
class AgentData {
    ...
    static fromName(name) {
        return new AgentData(name, 999, // difficulty: number;
        "1d1", // numberAppearing: string;
        new Array(), // attacks: string[];
        0, // baseLevel: number;
