...
class DemoData_MoversTests extends TestFixture
{
	...
	buildEntityDefnGroups(): void
	{
		...
		var demoData_Activities =
			new DemoData_Activities(null, universe.randomizer);
		...
		var moverGroupsAndCorpsesGroup =
			demoData_Movers.buildEntityDefnGroups_MoverGroupsAndCorpsesGroup
			(
				visualGetByName, activityDefnsByName, itemCategories
			);

		Assert.isNotNull(moverGroupsAndCorpsesGroup);
		Assert.areEqual(2, moverGroupsAndCorpsesGroup.length);

		var entityDefnGroupsMovers =
			moverGroupsAndCorpsesGroup[0] as EntityDefnGroup[];
		var entityDefnGroupCorpses =
			moverGroupsAndCorpsesGroup[1] as EntityDefnGroup;
